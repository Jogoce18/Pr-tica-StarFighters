...
function getStarCount(fighterRepos: any[]) {
...
export async function battle(firstUser: string, secondUser: string) {
  ...
  const firstUserStarCount = getStarCount(firstUserRepos);
  const secondUserStarCount = getStarCount(secondUserRepos);
...
async function getBattleResult(firstFighter: any, secondFighter: any, firstUserStarCount: number, secondUserStarCount: number) {
  if (firstUserStarCount > secondUserStarCount) {
    await updateStatsUser(firstFighter.id, secondFighter.id);
  ...
  if (secondUserStarCount < firstUserStarCount) {
    await updateStatsUser(secondFighter.id, firstFighter.id);
    return {
...
async function updateStatsUser(winnerId: number, loserId: number) {
  await fighterRepository.updateStatistics(winnerId, "wins");
  await fighterRepository.updateStatistics(loserId, "losses");
}

async function updateDrawStats(firstFighterId: number, secondFighterId: number) {
  await fighterRepository.updateStatistics(firstFighterId, "draws");
  await fighterRepository.updateStatistics(secondFighterId, "draws");
}
